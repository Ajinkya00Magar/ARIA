...
import { Terminal as TermIcon, AlertCircle, FileText, Plus, X } from 'lucide-react';
...
export function BottomPanel({ activePanel, onChangePanel, onClose }: BottomPanelProps) {
  ...
  const [termTabs, setTermTabs] = useState<TermTab[]>([{ id: `term-1`, label: 'Terminal 1' }]);
  ...
  const handleNewTerminal = () => {
    termCounter += 1;
    const newTab: TermTab = { id: `term-${termCounter}`, label: `Terminal ${termCounter}` };
    setTermTabs((prev) => [...prev, newTab]);
    setActiveTermId(newTab.id);
    // Ensure we are on the terminal panel
    if (activePanel !== 'terminal') onChangePanel('terminal');
  };
  ...
  const handleCloseTab = (id: string) => {
    setTermTabs((prev) => {
      ...
      if (next.length === 0) {
        // Spawn a fresh default tab instead of leaving nothing
        termCounter += 1;
        const fresh: TermTab = { id: `term-${termCounter}`, label: `Terminal ${termCounter}` };
  ...
  return (
    <div className="flex flex-col h-full bg-[#161616] border-t border-[#393939]">
      {/* Tab Bar */}
      <div className="flex items-center border-b border-[#393939] bg-[#1e1e1e] shrink-0 overflow-x-auto hide-scrollbar">
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={cn(
              'flex items-center gap-1.5 px-4 py-1.5 text-xs border-b-2 transition-colors shrink-0',
              activePanel === id
        ...
        {activePanel === 'terminal' && (
          <div className="h-4 w-px bg-[#393939] mx-1 shrink-0" />
        )}

        {/* Terminal instance tabs */}
        {activePanel === 'terminal' && termTabs.map((tab) => (
          <div
            key={tab.id}
            className={cn(
              'flex items-center gap-1.5 pl-3 pr-1 py-1.5 text-xs shrink-0 cursor-pointer transition-colors group',
              activeTermId === tab.id
                ? 'bg-[#161616] text-[#f4f4f4] border-b-2 border-[#4589ff]'
                : 'text-[#8d8d8d] hover:text-[#c6c6c6] hover:bg-[#262626] border-b-2 border-transparent',
            )}
          ...
          >
            ...
            {termTabs.length > 1 && (
              <button
                ...
                className="opacity-0 group-hover:opacity-100 ml-0.5 hover:text-red-400 rounded transition-opacity"
              >
                <X className="h-3 w-3" />
              </button>
            )}
        ...
        {activePanel === 'terminal' && (
          <button
            ...
            className="ml-1 p-1 text-[#8d8d8d] hover:text-white rounded hover:bg-[#393939] transition-colors shrink-0"
            title="New Terminal"
          >
        ...
        <button
          type="button"
          onClick={onClose}
          className="ml-auto mr-2 p-1 text-[#8d8d8d] hover:text-white rounded hover:bg-[#393939] transition-colors shrink-0"
          title="Close Panel"
        >
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        {activePanel === 'terminal' && termTabs.map((tab) => (
          <div key={tab.id} className={cn('w-full h-full', activeTermId === tab.id ? 'block' : 'hidden')}>
            <IntegratedTerminal workspacePath={currentWorkspace?.path || '.'} visible={activeTermId === tab.id} />
          </div>
        ))}
...
function OutputPanel() {
  return (
    <div className="flex flex-col h-full p-3 font-mono text-xs text-[#8d8d8d]">
      <p className="text-[#4589ff]">ARIA — Output</p>
      <p className="mt-1">ARIA ready. Start a conversation to see agent output here.</p>
    </div>
  );
}
